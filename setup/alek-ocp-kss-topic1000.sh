...
export SETUP_ID=alek-ocp-kss-topic1000
...
kubectl apply -f setup/kafka-source1000.yaml
echo "Wait for Kafka source to get ready"
kubectl wait kafkasources.sources.knative.dev/kafka-src1k --for=condition=Ready --timeout=300s
