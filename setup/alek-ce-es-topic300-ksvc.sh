...
export SETUP_ID=alek-ce-es-p300
...
kubectl apply -f setup/kafka-source-es300-ksvc.yaml
echo "Waiting for Kafka source to get ready"
kubectl wait kafkasources.sources.knative.dev/kafka-es300 --for=condition=Ready --timeout=300s

source ./setup/es.sh
export KAFKA_TOPIC=p300
